...
# Delete seeded designs for ReportTypeID = 1
$delCmd1 = $conn.CreateCommand()
$delCmd1.CommandText = "DELETE FROM ReportDesigns WHERE ReportID IN (SELECT ReportID FROM Reports WHERE ReportTypeID = 1)"
$delCmd1.ExecuteNonQuery() > $null
Write-Host "Deleted designs for LİSTE (Type 1) reports."

# Get all TASARIM (ReportTypeID = 2)
$cmd = $conn.CreateCommand()
$cmd.CommandText = "SELECT ReportID, ReportKey, ReportName, ISNULL(IsPosReport, 0) FROM Reports WHERE ReportTypeID = 2"
